#!/usr/bin/env python3
# Vendored from ai-marketing-claude (MIT (c) Zubair Trabzada) — see repo NOTICE.
# Only additive changes: a network-safety wrapper (SSRF/TLS) in front of the
# fetchers; the HTML parsing/scoring logic below is unchanged from upstream.
"""
        ...
        if tag == "title":
            ...
            if self._script_type == "application/ld+json":
                try:
                    ...
                    schema = None
                # keep only JSON-LD objects; flatten @graph; ignore scalars
...
MAX_FETCH_BYTES = 3_000_000  # cap body to avoid unbounded downloads


def _ssl_context():
    """Verify TLS certs by default; only skip if the caller opts in explicitly."""
...
def _url_allowed(url):
    """Allow only public http(s) URLs. To audit your own localhost/intranet,
    set AAK_ALLOW_PRIVATE_URLS=1 (deliberate opt-in; guards against SSRF)."""
    parsed = urlparse(url)
...
def fetch_page(url):
    ...
    return _safe_get(url, timeout=15, user_agent=(
...
def analyze(url):
    ...
    # coarse 0-10 heuristic signals, NOT the /audit 0-100 rubric
    page_results["signal_scores_0_to_10"] = scores
    page_results["heuristic_signal_0_to_10"] = round(sum(scores.values()) / len(scores), 1)
    page_results["score_scale"] = "0-10 heuristic signal from observable page elements; not a marketing verdict"
...
def main():
    ...
    if not re.match(r"^https?://", url):
        url = "https://" + url
    if not _url_allowed(url):
        print(json.dumps({"error": "URL rejected: only public http(s) addresses are scanned. "
