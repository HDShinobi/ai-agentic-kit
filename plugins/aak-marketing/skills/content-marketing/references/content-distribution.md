...
> **⚠️ Directional dated figures.** The "Facebook organic reach fell from ~20% to under 2%" claim and the Platform Half-Lives table are illustrative, time-sensitive estimates, not current measured values. Treat them as directional (the *pattern* — rented channels decay and get throttled, half-lives differ by orders of magnitude — is the durable lesson), and re-check the specific numbers before quoting them to a client.
...
Cross-links: ORB channel-type playbook lives in **launch-strategy**; atomization/repurposing workflows (podcast → clips, blog → thread) live in **social-media-expert**. This file consolidates the strategy that ties them together—don't re-derive ORB from scratch here.
...
For each major piece, produce (see **social-media-expert** for the platform-native execution):
...
- **launch-strategy** — ORB channel-type playbook and launch-day distribution
- **social-media-expert** — atomization/repurposing workflows and platform-native execution
- **email-marketing** — the owned channel that converts distributed attention
- **geo-fundamentals** — making owned content citable by LLMs (another distribution surface)
