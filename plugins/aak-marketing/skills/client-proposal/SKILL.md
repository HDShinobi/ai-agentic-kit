...
- Invoked when the user asks to build a client proposal (often right after `/aak-marketing:audit`).
...
**If audit data exists:** Check for any previous `/aak-marketing:audit` results. If found, automatically incorporate the findings into the Situation Analysis section for a data-backed proposal.
...
Present your analysis of the client's current marketing. This is where audit data from `/aak-marketing:audit` is invaluable.
...
3. **Competitive Landscape** -- How they compare to competitors (from the `competitor-teardown` skill if available)
