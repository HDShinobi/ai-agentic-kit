...
- **robots.txt with an explicit AI-crawler stance** — name the major AI crawlers and state your policy, rather than leaving it to be assumed (see the bot-access table in ai-seo-playbook.md for the allow/block list).
- **A sitemap that loads and parses cleanly.**
- **llms.txt at the domain root** (see Machine-Readable Files in ai-seo-playbook.md).
...
- **Valid, substantive structured data** (JSON-LD — see the `schema-markup` skill).
...
- **Treat any citation-share statistic as dated.** The "Reddit = 1.8% of ChatGPT citations" class of stats (including the ones in this skill) are snapshots — check the date, and verify against your own citation monitoring (the DIY monitoring loop in ai-seo-playbook.md) before betting budget on them.
