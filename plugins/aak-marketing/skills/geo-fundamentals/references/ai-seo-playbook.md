...
> **⚠️ The grafted `references/*.md` guides supersede this playbook's own crawler-name table and rough RAG-weight numbers.** Use the current official crawler docs (ClaudeBot / anthropic-ai / Claude-SearchBot, GPTBot / ChatGPT-User, PerplexityBot, Google-Extended, Bingbot, etc.) for authoritative user-agent names, and `references/platform-ranking-factors.md` for per-platform selection factors — the crawler names and weightings below are illustrative and drift over time.
> **⚠️ Every citation-share %, lift, and study figure inside is a DATED observational snapshot, not a causal benchmark.** Numbers like Wikipedia 7.8% of ChatGPT citations, "~45% of searches" showing AI Overviews, the 132% / 89% Google AI Overview boosts, and the Amsive / Princeton GEO / SE Ranking / ZipTie / Scrunch / SimilarWeb studies are point-in-time observations that shift overnight with model and retrieval updates — verify against current sources and your own monitoring before betting budget on them.
> **⚠️ `community-marketing` referenced inside this material is not a skill in this kit** — the nearest home is `social-media-expert`.
> **⚠️ Any tool-registry or tool-integration links are upstream-repo paths not shipped here** — treat "tools registry" and named connectors (`semrush`, `ahrefs`, `gsc`, `ga4`, etc.) as plain labels, not reachable files.
...
- Create YouTube content for key how-to queries — models don't watch the video, they read the text layer around it; see [references/youtube-ai-citations.md](youtube-ai-citations.md) for the full anatomy (transcript, captions, chapters, description, pinned comment)
...
**Audit this layer first**: [references/agent-readiness.md](agent-readiness.md) — the access/discovery/parseability checklist, free scoring tools (`npx is-agentic`, Frase's checker), Markdown content negotiation + `Link` headers, `llms-full.txt`, and the emerging agent-*actionable* layer (WebMCP).
...
Content with proper schema shows 30-40% higher AI visibility on non-Google AI engines. **Google's note**: structured data is "not required for generative AI search" but is recommended for overall SEO strategy. For implementation, use the **schema-markup** skill.
...
- **seo-fundamentals**: For traditional technical and on-page SEO audits
- **schema-markup**: For implementing structured data that helps AI understand your content
- **content-marketing**: For planning what content to create
