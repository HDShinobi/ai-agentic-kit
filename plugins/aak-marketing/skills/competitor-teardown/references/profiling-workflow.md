...
> **⚠️ Never invent data.** Funding, review ratings, organic-traffic, and backlink fields are only as good as their source. When a value can't be sourced, write **"Not available"** — do not estimate a number to fill the cell. Every figure you do include must carry its source and an as-of date, and every inference or estimate must be explicitly labeled as such. An honest gap is more useful than a fabricated fact.
> **ℹ️ `prospecting` has no dedicated skill in this kit.** Where the workflow below points to a `prospecting` skill (Related Skills), the nearest capability here is **`lead-gen-scraper`** (list-building / scraping) — see the inline note there.
...
Use DataForSEO MCP tools to gather quantitative competitive intelligence. Save each raw response as JSON to `competitor-profiles/raw/<competitor-slug>/<YYYY-MM-DD>/seo/<endpoint-name>.json` before parsing it into the profile. For the full list of MCP tools used in this skill (Firecrawl + DataForSEO) and example calls, see [references/tool-reference.md](tool-reference.md).
...
**For the full profile and summary templates**: See [references/templates.md](templates.md)
...
- **prospecting**: For broader list-building qualification (this skill does deep research on specific accounts; prospecting builds the initial list). <!-- KIT NOTE: no `prospecting` skill in this kit — nearest is `lead-gen-scraper` (list-building / scraping). -->
- **customer-research**: For mining reviews and community sentiment in depth
- **content-marketing**: For using competitor content gaps to plan your own content
- **seo-fundamentals**: For auditing your own site relative to competitors
- **sales-enablement**: For turning profiles into battle cards and sales collateral
- **ppc-advertising**: For analyzing competitor ad strategies (prefer `aak-ads` if that plugin is enabled — it supersedes the generalist paid-media skills for anything PPC/ad-account)
